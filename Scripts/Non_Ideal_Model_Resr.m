...
%   oscillator with inductor equivalent series resistance only. In order
%   to ensure consistency and error proof derivations, the expressions were
%   derived using three different approaches:
...
syms L C1 R C2 gm Rs w w0 positive 
syms s
...
w0 = sqrt((R*(C1 + C2) + C1*Rs)/(C1*C2*L*R));
w0approx = sqrt(1/(C1*C2/(C1 + C2)*L))
cond = expand(subs(re, w, w0));
condapprox = expand(subs(re, w, w0approx))
%% Derivations of w0approx and condapprox with loop gain approach

Z_L = simplify(X2*(X1 + X3)/(X1 + X2 + X3), "Steps", 1000);
beta = (X3)/(X1 + X3);
Z = Z_L*R/(Z_L + R);
Av = simplify(gm*Z/(1 + gm*Z), "Steps", 1000);
...
A = [R + 1/(s*C2) + gm*R*1/(s*C2), -(s*gm*R*L + gm*R*Rs + 1/(s*C2)*(gm*R + 1));
     -1/(s*C2), s*L + Rs + + 1/(s*C1) + 1/(s*C2)];
